    ...
    stages {
        ...
        stage('Deploy') {
            steps {
                ...
                docker run -d --name mysql --network task2-net -e MYSQL_ROOT_PASSWORD=${YOUR_NAME} longehdocker/task2-db
                docker run -d --name flask-app --network task2-net -e MYSQL_ROOT_PASSWORD=${YOUR_NAME} longehdocker/task2jenk
